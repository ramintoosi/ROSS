...
class UserAccount():
    ...
    def get_sorting_result(self):
        if not (self.access_token == None):
            response = requests.get(self.url + '/sorting_result', headers={'Authorization': 'Bearer ' + self.access_token})
            if response.ok:
                b = io.BytesIO()
                b.write(response.content)
                b.seek(0)
                ...
                if response.ok:
                    ...
                    b.seek(0)
                    d = np.load(b, allow_pickle=True)
                    return {'stat': True, 'clusters': d['cluster_index']}
            return {'stat': False, 'message': response.json()['message']}
        return {'stat': False, 'message': 'Not Logged In!'}
    ...
    def start_resorting(self, resort_data):
        sel_cl = resort_data
        buffer = io.BytesIO()
        np.savez_compressed(buffer, selected_clusters=sel_cl)
        buffer.seek(0)
        clusters_bytes = buffer.read()
        buffer.close()
        if not(self.access_token == None):
            response = requests.put(self.url + '/resort', headers={'Authorization': 'Bearer ' + self.access_token},
                                    data=clusters_bytes)
            ...
            if response.ok:
                ...
                self.refresh_jwt_token()
                response = requests.put(self.url + '/resort', headers={'Authorization': 'Bearer' + self.access_token}, data=clusters_bytes)
                if response.ok:
                    return {'stat': True, 'message': 'success'}
            return {'stat': False, 'message': response.json()["message"]}
        return {'stat': False, 'message': 'Not Logged In!'}
    ...
    def save_sort_results(self, clusters):
        if not (self.access_token == None):
            buffer = io.BytesIO()
            np.savez_compressed(buffer, clusters=clusters)
            buffer.seek(0)
            clusters_bytes = buffer.read()
            buffer.close()
            response = requests.put(self.url + '/sorting_result', headers={'Authorization': 'Bearer ' + self.access_token},
                                    data=clusters_bytes)
            if response.ok:
                return {'stat': True, 'message': 'success'}
            elif response.json()["message"] == 'The token has expired.':
