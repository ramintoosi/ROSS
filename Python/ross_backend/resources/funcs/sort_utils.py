...
def matlab_round(num):  # matlab rounds 0.5 to 1 and -0.5 to -1, but python rounds to even numbers
    res = (int(num > 0) - int(num < 0)) * int(abs(num) + 0.5)
    return res


def spikeFiltering(spike_mat, config):
# This functions does statistcal filtering on detected spikes by thresholding on waveform std and mean, and returns a
# logical matrix containing location of detected outliers(L);
    Max_std = config.max_std
    Max_mean = config.max_mean

    spike_std = np.std(spike_mat, axis=1)
    spike_mean = np.mean(spike_mat, axis=1)
    ...
    if sum(L) / len(L) > config.max_outliers:
        # limiting number of detected outliers to maximum allowed percent by sorting them based on criterion
        criterion = spike_std / np.std(spike_std) + abs(spike_mean) / np.std(spike_mean)
        criterion_s = np.sort(criterion[::-1])
        L = criterion > criterion_s(np.ceil(len(L) * config.max_outliers))

    return L


def spikeAlignment(spike_mat, spike_time, config):
    ...
    n_peaks = config.num_peaks
    n_bin = config.histogram_bins
    ind_min = np.zeros((n_spike, 1))
    ind_max = np.zeros((n_spike, 1))
    amp_min = np.zeros((n_spike, 1))
    amp_max = np.zeros((n_spike, 1))
    # finding min and max indices for each spike
    for i in range(len(spike_mat)):
        # cubic spline interpolation, S contains values corresponding to the query points in linspace(1, n_feat, n_upsample)
        # S = interp.CubicSpline(np.arange(n_feat) + 1, spike_mat[i, :], np.linspace(1, n_feat, n_upsample))


        # spline = interp.UnivariateSpline(np.arange(n_feat) + 1, spike_mat[i, :])
        # S = spline(np.linspace(1, n_feat, n_upsample))

        cs = CubicSpline(np.arange(n_feat) + 1, spike_mat[i, :])
        S = cs(np.linspace(1, n_feat, n_upsample))

        # cs = CubicSpline(cen_neg, freq_neg)
        # s_neg = cs(np.linspace(min(cen_neg), max(cen_neg), 5 * n_bin))


        ind_min[i] = int(np.argmin(S))
        amp_min[i] = S[int(ind_min[i][0])]
        ind_max[i] = int(np.argmax(S))
        amp_max[i] = S[int(ind_max[i][0])]

    if config.compare_mode == 'index':
        # grouping samples based on index
        ind_neg = ind_min < ind_max
    elif config.compare_mode == 'magnitude':
        # grouping samples based on magnitude
        ind_neg = abs(amp_min) < abs(amp_max)
    else:
        raise Exception('Comparison Mode in alignment is not supported')
    # indices of first group minimums
    ind_min_neg = ind_min[ind_neg]
    # calculating samples histogram
    freq_neg, cen_neg_ = np.histogram(ind_min_neg + 1, n_bin)
    cen_neg = cen_neg_[:-1] + np.diff(cen_neg_)/2
    ...
    # peaks, properties = signal.find_peaks(s_neg, height=0.0001, distance=max_spline_align_shift / 2)
    peaks = detect_peaks(s_neg, mpd=max_spline_align_shift/2)
    peak_heights = s_neg[peaks]
    # print(peaks, properties.keys())
    # peak_heights = properties['peak_heights']
    loc_peaks_neg = [x for _,x in sorted(zip(peak_heights, peaks), reverse=True)]
    ...
    if len(loc_peaks_neg) > n_peaks:
        loc_peaks_neg = loc_peaks_neg[:n_peaks]
    ...
    # calculating samples histogram
    freq_pos, cen_pos_ = np.histogram(ind_max_pos + 1, n_bin)
    cen_pos = cen_pos_[:-1] + np.diff(cen_pos_)/2
    # cubic spline interpolation of histogram
    cs = CubicSpline(cen_pos, freq_pos)
    s_pos = cs(np.linspace(min(cen_pos), max(cen_pos), 5 * n_bin))

    # further smoothing
    s_pos = rt_smoother(s_pos, 50)
    # finding local maxima of the smoothed histogram

    peaks = detect_peaks(s_pos, mpd=max_spline_align_shift/2)
    peak_heights = s_pos[peaks]
    loc_peaks_pos = [x for _, x in sorted(zip(peak_heights, peaks), reverse=True)]
    # adding to resolution to find peaks more precisely
    x = np.linspace(min(cen_pos), max(cen_pos), 5 * n_bin)
    loc_peaks_pos = np.round(x[loc_peaks_pos])
    # choosing first "n_peaks" peaks (peaks are sorted in descending order)
    if len(loc_peaks_pos) > n_peaks:
        loc_peaks_pos = loc_peaks_pos[:n_peaks]
...
def rt_smoother(y, win_length=5):
    ...
    win = signal.gaussian(win_length, std=(win_length - 1)/(2 * win_alpha))
    win = win/sum(win)
    # ps = np.convolve(y, win, 'same')
    npad = len(win) - 1

    full = np.convolve(y, win, 'full')

    first = npad - npad // 2

    ps = full[first:first + len(y)]
